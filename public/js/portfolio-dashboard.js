(function () {
    const config = window.PORTFOLIO_DASHBOARD_CONFIG || {};
    const API_URL = config.apiUrl || '';
    const MERCURE_SUBSCRIBE_URL = config.mercureSubscribeUrl || null;
    ...
    let chart = null;
    let currentPeriod = { minutes: DEFAULT_PERIOD_MINUTES };

    /**
     * Formats a number as USDT amount (2 decimal places).
    ...
    /**
     * Formats a number as percent with sign (+/-) and two decimal places.
     * @param {number|null|undefined} n
     * @returns {string}
     */
    ...
    /**
     * Shows one of the chart state blocks (loading / empty / error / chart), hides the rest.
     * @param {string} id element id: chart-state-loading, chart-state-empty, chart-state-error, chart-container
     */
     ...
     * @param {{ minutes?: number, hours?: number }} period object with minutes or hours
            ...
            .then(function (data) {
                ...
                if (historyBlock && historyList && historyPeriodLabel) {
                    ...
                    historyList.innerHTML = data.slice().reverse().map(function (d) {
                        ...
                        return '<li class="history-item"><span class="history-item__time">' + escapeHtml(timeStr) + '</span> <span class="history-item__value">' + escapeHtml(formatUsdt(d.amount_usdt)) + '</span></li>';
                    }).join('');
                    historyList.classList.remove('hidden');
                }
            })
    ...
    function periodLabel(period) {
        if (period.minutes != null) { return period.minutes === DEFAULT_PERIOD_MINUTES ? DEFAULT_PERIOD_MINUTES + ' min' : period.minutes + ' min'; }
        if (period.hours === 1) { return '1 h'; }
        if (period.hours === MAX_HOURS) { return MAX_HOURS + ' h'; }
    ...
    function getPeriodFromUrl() {
        ...
        const h = p.get('hours');
        if (m !== null && m !== '') { const n = parseInt(m, 10); if (n >= 1 && n <= MAX_MINUTES) { return { minutes: n }; } }
        if (h !== null && h !== '') { const n = parseInt(h, 10); if (n >= 1 && n <= MAX_HOURS) { return { hours: n }; } }
        return null;
    }

    /**
     * Writes selected period to URL (minutes or hours) via pushState without reload.
     * @param {{ minutes?: number, hours?: number }} period object with minutes or hours
     */
    ...
    /**
     * Highlights the active period button by data-minutes/data-hours to match current period.
     * @param {{ minutes?: number, hours?: number }} period object with minutes or hours
     */
    ...
    /**
     * Applies selected period: updates currentPeriod, buttons, label and triggers data load.
    ...
    /**
     * Shows toast notification for TOAST_DURATION_MS ms.
    ...
    function showToast(text) {
        ...
        setTimeout(function () { toast.classList.add('hidden'); }, TOAST_DURATION_MS);
    }

    const mercureStateEl = document.getElementById('mercure-state');
    /**
     * Updates Mercure status indicator (ON/OFF and class).
    ...
    applyPeriod(getPeriodFromUrl() || { minutes: DEFAULT_PERIOD_MINUTES });
})();
